...
-- take the first element of a cons
class First ls x | ls -> x
instance First Nil Nil
instance First (Cons x rest) x

-- take the rest of a cons
class Rest ls x | ls -> x
instance Rest Nil Nil
instance First (Cons x rest) rest
...
class AnyTrue ls t | ls -> t
instance AnyTrue Nil False
instance AnyTrue (Cons True rest) True
instance (AnyTrue ls t) => AnyTrue (Cons False ls) t
...
{- the Peano axioms:
- axioms for the natural numbers from Peano
- 0 is a natural number
- for every natural number x, x = x; reflexivity
- for all natural numbers x and y, if x = y then y = x; symmetry
- for all natural numbers x, y, and z, if x = y and y = z, then y = z. transitivity!
- for all a and b, if b is a natural number and a = b, then a is also a natural number. (closed under equality)

arithmetic properties of the natural numbers:
- for every natural n, S(n) (the 'successor' of n) is a natural number
- for all natural numbers m and n, m = n iff S(m) = S(n) (injection)
- for every natural number n, S(n) = 0 is false; no natural number has a successor of 0
-}
...
-- map applied function over the list
...
-- filter list with applyable predicate function
...
instance (Apply f x t, Filter f xs ys, AppendIf t x ys zs) => Filter f (Cons x xs) zs

--- n-queens problem from the article ---
data Queen x y -- a queen is defined by board coordinates
data QueenX x
instance Apply (QueenX x) y (Queen x y)

-- listof queens in row x with y from 0 to n
...
instance ( PeanoEqual ax bx xeq
         ...
         , Or xyeq deq res) => Threatens (Queen ax ay) (Queen bx by) res
